...
const SandpackCustom = ({ sandpackDone }: ISandpackProps) => {
...
interface Props {
  ...
  fixBug: (error: { message: string; stack: string }) => void;
}
...
export default function PreviewBox({
  ...
  useEffect(() => {
    ...
    document
      .querySelector(".lc-simulator-content-frame")
  ...
  useEffect(() => {
    if (appState === AppState.CODE_READY) {
      if (
        ...
        const codeUid = setHtmlCodeUid(
          generatedCodeConfig,
          code.replaceAll("@/components", "components")
        );
  ...
  return (
    <div className="border-[4px] border-black rounded-[10px] shadow-lg h-full w-full">
      {showDebug && (
        <span
          onClick={() => {
            fixBug(errorObj);
