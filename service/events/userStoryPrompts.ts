const INSURANCE_PRODUCT_LIST_PROMPT = `生成一个保险产品列表页面，保险产品列表页面它包含了以下几个主要部分：
  头部区域（Header Section）:
  头部区域分为上下两部分，
        上面部分有以下元素：
...
const INSURANCE_PRODUCT_DETAIL_PROMPT = `生成一个保险产品详情页面, 需求如下：
  Header：垂直居中
      第一行：（元素水平排列）
  品牌标志：图片，居左展示。
  搜索框：文本框
...
const INSURANCE_PRICE_CALCULATE_PROMPT = `生成一个保险投保页面，投保页信息包括：
  Header：垂直居中
  - 第一行：（元素水平排列）
    - 品牌标志：图片，居左展示。
    - 搜索框：文本框
...
export const USER_STORY_MAP = {
  ...
  insurance_product_detail: INSURANCE_PRODUCT_DETAIL_PROMPT,
  insurance_price_calculate: INSURANCE_PRICE_CALCULATE_PROMPT,
};
