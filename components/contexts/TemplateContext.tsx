...
export const TemplateContext =
  createContext<templatetContextType>(initialValue);
export default function TemplatetProvider({
  children,
}: {
  children: ReactNode;
}) {
  ...
  function addTemplate(template: TemplateType) {
    ...
    setTemplateList((prevState) => {
      ...
      const curIndex = newTemplates.findIndex(
        (curTemplate) => curTemplate.id === template.id
      );
      if (curIndex >= 0) {
